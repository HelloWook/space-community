...
신규 사용자가 기존에 사용하던 소셜 계정(Google, GitHub, Kakao)을 이용하여 별도의 비밀번호 설정 없이 빠르게 서비스에 가입하고 로그인한다.
...
4. **Given** 사용자가 로그인 화면에 있을 때, **When** "Kakao로 로그인" 버튼을 클릭하고 Kakao 계정 인증을 완료하면, **Then** 서비스 계정이 생성되거나 기존 계정으로 로그인된다
...
사용자가 소셜 로그인 시 이미 다른 소셜 제공자로 가입된 동일 이메일이 존재하는 경우, 안내를 받고 계정을 연동하거나 별도 조치를 취할 수 있다.

**Why this priority**: 엣지 케이스이지만 사용자 혼란을 방지하고 계정 보안을 유지하는 데 중요하다. P1, P2가 완료된 후 처리한다.

**Independent Test**: 동일 이메일로 다른 소셜 제공자를 통해 로그인을 시도하여 안내 메시지와 계정 연동 제안이 표시되는지 확인

**Acceptance Scenarios**:

1. **Given** Google(email: user@example.com)로 가입된 사용자가 있을 때, **When** 동일 이메일의 GitHub 계정으로 로그인을 시도하면, **Then** "이미 Google로 가입된 이메일입니다. 기존 계정에 연동하시겠습니까?" 안내가 표시된다
2. **Given** 이메일 충돌 안내를 받은 사용자가, **When** "기존 계정에 연동" 옵션을 선택하고 기존 소셜 제공자로 본인 인증을 완료하면, **Then** 두 소셜 계정이 하나의 서비스 계정에 연동된다
...
- **FR-001**: 시스템은 Google, GitHub, Kakao 세 가지 소셜 제공자를 통한 로그인을 지원해야 한다 (MUST)
- **FR-002**: 소셜 로그인 시 서비스에 계정이 없으면 자동으로 신규 계정을 생성해야 한다 (MUST)
- **FR-003**: 소셜 로그인 시 이미 계정이 있으면 기존 계정으로 로그인되어야 한다 (MUST)
- **FR-004**: 사용자는 로그인 화면에서 지원되는 소셜 제공자 버튼을 확인할 수 있어야 한다 (MUST)
...
- **사용자(User)**: 서비스 이용자. 하나의 이메일과 하나 이상의 소셜 연동을 보유
- **소셜 연동(Social Connection)**: 사용자와 소셜 제공자 간의 연결. 소셜 제공자 식별자, 제공자 유형, 연동 일시 등을 포함
- **소셜 제공자(Social Provider)**: 외부 인증 서비스(Google, GitHub, Kakao). 제공자별 인증 흐름과 반환 데이터가 다름
...
- 사용자는 Google, GitHub, Kakao 중 하나 이상의 소셜 계정을 이미 보유하고 있다
